...
@interface AirportbriefingTableViewController ()
{
//    Airport *airportWX;
    NSURL *fileURL;
    NSArray *result;
...
@implementation AirportbriefingTableViewController
{
    ReaderViewController *readerViewController;

}
@synthesize btnAircraft,btnShowMap,btnBack;
...
- (void)viewDidLoad {
    ...
    btnShowMap = [[UIBarButtonItem alloc]initWithTitle:@"| MAP |" style:UIBarButtonItemStylePlain target:self action:@selector(showAirportOnMAP:)];
    [self.navigationItem setRightBarButtonItems:[NSArray arrayWithObjects:btnAircraft,btnShowMap, nil]];
    ...
    self.lblRFFNotes.text = [self.airport.rffnotes length] ? self.airport.rffnotes : @"";
    ...
    if ([btnAircraft.title isEqualToString:@"A320/A321"]) {
        ...
        self.txtViewNOTES.text = [self.airport.note32x length] ? self.airport.note32x : @"NIL";
    ...
    if ([btnAircraft.title isEqualToString:@"A332/A33F"]) {
        ...
        self.lblPEG.text = [@"PEG: " stringByAppendingString:[self.airport.peg length] ? self.airport.peg : @"TBA"];
        self.txtViewNOTES.text = [self.airport.note332 length] ? self.airport.note332 : @"NIL";
    ...
    if ([btnAircraft.title isEqualToString:@"A333"]) {
        ...
        self.lblPEG.text = [@"PEG: " stringByAppendingString:[self.airport.peg length] ? self.airport.peg : @"TBA"];
        self.txtViewNOTES.text = [self.airport.note333 length] ? self.airport.note333 : @"NIL";
    ...
    if ([btnAircraft.title isEqualToString:@"A345"]) {
        ...
        self.lblPEG.text = [@"PEG: " stringByAppendingString:[self.airport.peg length] ? self.airport.peg : @"TBA"];
        self.txtViewNOTES.text = [self.airport.note345 length] ? self.airport.note345 : @"NIL";
    ...
    if ([btnAircraft.title isEqualToString:@"A346"]) {
        ...
        self.lblPEG.text = [@"PEG: " stringByAppendingString:[self.airport.peg length] ? self.airport.peg : @"TBA"];
        self.txtViewNOTES.text = [self.airport.note346 length] ? self.airport.note346 : @"NIL";
    ...
    if ([btnAircraft.title isEqualToString:@"A350"]) {
        ...
        self.lblPEG.text = [@"PEG: " stringByAppendingString:[self.airport.peg length] ? self.airport.peg : @"TBA"];
        self.txtViewNOTES.text = [self.airport.note350 length] ? self.airport.note350 : @"NIL";
    ...
    if ([btnAircraft.title isEqualToString:@"A380"]) {
        ...
        self.lblPEG.text = [@"PEG: " stringByAppendingString:[self.airport.peg length] ? self.airport.peg : @"TBA"];
        self.txtViewNOTES.text = [self.airport.note380 length] ? self.airport.note380 : @"NIL";
    ...
    if ([btnAircraft.title isEqualToString:@"B777/B77F"]) {
        ...
        self.lblPEG.text = [@"PEG: " stringByAppendingString:[self.airport.peg length] ? self.airport.peg : @"TBA"];
        self.txtViewNOTES.text = [self.airport.note777 length] ? self.airport.note777 : @"NIL";
    ...
    if ([btnAircraft.title isEqualToString:@"B787"]) {
        ...
        self.lblPEG.text = [@"PEG: " stringByAppendingString:[self.airport.peg length] ? self.airport.peg : @"TBA"];
        self.txtViewNOTES.text = [self.airport.note787 length] ? self.airport.note787 : @"NIL";
    }

    
    NSString *strICAO = self.airport.icaoidentifier;
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"icaoidentifier ==  %@", strICAO];
    result = [[User sharedUser].arrayWXNotams filteredArrayUsingPredicate: predicate];
    if ([result count] != 0) {
        ...
        self.airportWX = [[User sharedUser].arrayWXNotams objectAtIndex:indexOfTheObject];
    }
    if ([self.airportWX.strMETAR length] == 0) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell == self.tblvcMETAR){
        ...
        return 150; //set the hidden cell's height to 0
        }
    }
    if(cell == self.tblvcNOTAMS) {
        if ([self.airportWX.strNOTAMS length] == 0) {
            return 30;
        } else {
            return 150; //set the hidden cell's height to 0
        }
...
-(IBAction)showAirportOnMAP:(UIButton *) sender
{
    [User sharedUser].bolCenterMap = YES;
    [User sharedUser].dblLatitudeMapCenter = _airport.latitude;
    [User sharedUser].dblLongitudeMapCenter = _airport.longitude;
    [self performSegueWithIdentifier: @"segShowMap" sender: self];
}
