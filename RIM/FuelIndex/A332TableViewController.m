...
#import "A332TableViewController.h"


@interface A332TableViewController ()

@end

@implementation A332TableViewController
...
- (void) calcTrimSet
{
    double c = 7-(7 * (([self.txtGWCG.text doubleValue]-21)/14));
    if (c < 0) {
    ...
    if ([self.txtGWCG.text doubleValue] >= 18 && [self.txtGWCG.text doubleValue] <= 21) {
    ...
    if ([self.txtGWCG.text doubleValue] > 38 || [self.txtGWCG.text doubleValue] < 18) {
...
-(IBAction)formatZFW:(id)sender
{
    if ([self.txtZFW.text length] >= 6) {
        if ([self.txtZFW.text intValue] > 170000) {
            UIAlertView *message = [[UIAlertView alloc] initWithTitle:@"Caution"
...
//    self.navBarStandartFuel.topItem.title = @"Non Standard Fuel Distribution A330-200";
    [self.navigationItem setTitle:@"Non Standard Fuel Distribution A330-200"];
...
{
    [self.navigationItem setTitle:@"Standard Fuel Distribution A330-200"];
...
//    self.txtTotalFuel.text = [[NSString alloc] initWithFormat:@"%d ", (int)self.sldFuel.value];
    ...
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+2400+30770 && intTotalFuel <= ((7300*intDensity)/1000)+((84000*intDensity)/1000)+2400)
    {
        ...
        self.txtCenterFuel.text = @"0";
        self.progFuelCenterTank.progress = 0;
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+((84000*intDensity)/1000)+2400 && intTotalFuel <= ((139090*intDensity)/1000))
    {
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%li ", ((42000*intDensity)/1000)];
