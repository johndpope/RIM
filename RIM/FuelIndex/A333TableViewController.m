...
#import "A333TableViewController.h"


@interface A333TableViewController ()

@end

@implementation A333TableViewController
...
- (void)viewDidLoad
{
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"A333top.png"]];
...
//    double dblFuel = (76561/(fltDensity));
...
- (void) calcTrimSet
{
    ...
    if ([self.txtGWCG.text doubleValue] >= 15 && [self.txtGWCG.text doubleValue] <= 21) {
        self.txtTrim.text = [[[NSString alloc]initWithFormat:@"%2.2f", 7.0] stringByAppendingString:@"° UP"];
    }
    else {
        if ([self.txtGWCG.text doubleValue] <= 38 && [self.txtGWCG.text doubleValue] >= 35) {
            self.txtTrim.text = [[[NSString alloc]initWithFormat:@"%2.2f", 0.0] stringByAppendingString:@"° DN"];
        }
    }
    if ([self.txtGWCG.text doubleValue] > 38 || [self.txtGWCG.text doubleValue] < 15) {
            }
...
-(IBAction)formatZFW:(id)sender
{
    if ([self.txtZFW.text length] >= 6) {
        if ([self.txtZFW.text intValue] > 175000) {
            UIAlertView *message = [[UIAlertView alloc] initWithTitle:@"Caution"
...
//    if ([self.navigationItem.title isEqualToString:@"Standard Fuel Distribution A330-300"]) {
...
-(IBAction)nonStdFuel:(id)sender
{
    ...
    [self.navigationItem setTitle:@"Non Standard Fuel Distribution A330-300"];
...
{
    [self.navigationItem setTitle:@"Standard Fuel Distribution A330-300"];
...
//    self.txtTotalFuel.text = [[NSString alloc] initWithFormat:@"%d ", (int)self.sldFuel.value];
    ...
    } else if ( intTotalFuel > 9000 && intTotalFuel <= ((7300*intDensity)/1000)+9000)
    {
        ...
        self.txtTrimFuel.text = @"0";
        self.progFuelTrimTank.progress = 0;
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+9000 && intTotalFuel <= ((7300*intDensity)/1000)+30770)
    {
        ...
        self.txtTrimFuel.text = @"0";
        self.progFuelTrimTank.progress = 0;
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+30770 && intTotalFuel <= ((7300*intDensity)/1000)+2400+30770)
    {
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%d ", 15386];
        self.progFuelInnerLeft.progress = (float)15386/(float)((32970*intDensity)/1000);
        self.txtInnerFuelRight.text = [[NSString alloc] initWithFormat:@"%d ", 15386];
        self.progFuelInnerRight.progress = (float)15386/(float)((32970*intDensity)/1000);
        self.txtOuterFuelLeft.text = [[NSString alloc] initWithFormat:@"%li ", ((3650*intDensity)/1000)];
        self.txtOuterFuelRight.text = [[NSString alloc] initWithFormat:@"%li ", ((3650*intDensity)/1000)];
        ...
        self.txtCenterFuel.text = @"0";
        self.progFuelCenterTank.progress = 0;
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+2400+30770 && intTotalFuel <= ((7300*intDensity)/1000)+((84000*intDensity)/1000)+2400)
    {
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%ld ", (intTotalFuel-(((7300*intDensity)/1000)+2400))/2];
        self.progFuelInnerLeft.progress = (float)((intTotalFuel-(((7300*intDensity)/1000)+2400))/2)/(float)((32970*intDensity)/1000);
        NSLog(@"%f", (float)((intTotalFuel-(((7300*intDensity)/1000)+2400))/2)/(float)((32970*intDensity)/1000));
        ...
        self.txtCenterFuel.text = @"0";
        self.progFuelCenterTank.progress = 0;
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+((84000*intDensity)/1000)+2400 && intTotalFuel <= ((76561)))
    {
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%li ", ((42000*intDensity)/1000)];
        ...
        NSInteger intFueled = (((42000*intDensity)/1000)*2)+(((3650*intDensity)/1000)*2)+2400;
//        self.txtTrimFuel.text = [[NSString alloc] initWithFormat:@"%1.0f ", (((intTotalFuel-intFueled)*7.01155)/100)+2400];
//        self.progFuelTrimTank.progress = (float)((((intTotalFuel-intFueled)*7.01155)/100)+2400)/(float)((6230*intDensity)/1000);
        self.txtTrimFuel.text = [[NSString alloc] initWithFormat:@"%1.0li ", ((intTotalFuel-intFueled))+2400];
        self.progFuelTrimTank.progress = (float)((intTotalFuel-intFueled)+2400)/(float)((6230*intDensity)/1000);
        
//        self.txtCenterFuel.text = [[NSString alloc] initWithFormat:@"%1.0f ",(intTotalFuel - intFueled) - ((intTotalFuel-intFueled)*7.01155)/100];
//        NSInteger intfuelonboard = (((42000*intDensity)/1000)*2) + (((3650*intDensity)/1000)*2) + 2400;
//        self.progFuelCenterTank.progress = (float)((intTotalFuel - intfuelonboard) - ((intTotalFuel-intfuelonboard)*7.01155)/100)/(float)((41560*intDensity)/1000);
    }
...
-(void)indexInnerTankLeft
{
    ...
    NSLog(@"%f", (float)intInnerFuel/(float)((42000*intDensity)/1000));
}
