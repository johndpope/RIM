...
#import "A343TableViewController.h"


@interface A343TableViewController ()

@end

@implementation A343TableViewController
...
#define RefSta 36.3495
#define LEMAC 34.5320
...
//    NSString *validRegEx;
//    if (textField.tag == 1) {
//        validRegEx =@"^[0-9.]*$"; //change this regular expression as your requirement
//    } else
//    validRegEx =@"^[0-9]*$"; //change this regular expression as your requirement
...
-(IBAction)formatZFWIndex:(id)sender
{
    ...
    if ([self.txtZFWIndex.text intValue] > 190) {
...
-(IBAction)formatInnerLeft:(id)sender
{
    ...
    if ([self.txtInnerFuelLeft.text intValue] > ((42000*intDensity)/1000)) {
        ...
        [message show];
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%li ", ((42000*intDensity)/1000)];
    }
...
-(IBAction)formatInnerRight:(id)sender
{
    ...
    if ([self.txtInnerFuelRight.text intValue] > ((42775*intDensity)/1000)) {
        UIAlertView *message = [[UIAlertView alloc] initWithTitle:@"Caution"
                                                          message:@"Max Quantity exceeded!"
                                                         delegate:nil
                                                cancelButtonTitle:@"OK"
        ...
        [message show];
        self.txtInnerFuelRight.text = [[NSString alloc] initWithFormat:@"%li ", ((42000*intDensity)/1000)];
    }
...
-(IBAction)formatOuterLeft:(id)sender
{
    ...
    if ([self.txtOuterFuelLeft.text intValue] > ((3650*intDensity)/1000)) {
        UIAlertView *message = [[UIAlertView alloc] initWithTitle:@"Caution"
                                                          message:@"Max Quantity exceeded!"
                                                         delegate:nil
                                                cancelButtonTitle:@"OK"
...
-(IBAction)formatCenter:(id)sender
{
    ...
    if ([self.txtCenterFuel.text intValue] > ((42420*intDensity)/1000)) {
        UIAlertView *message = [[UIAlertView alloc] initWithTitle:@"Caution"
                                                          message:@"Max Quantity exceeded!"
                                                         delegate:nil
                                                cancelButtonTitle:@"OK"
...
- (void)viewDidLoad
{
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"A340-300.png"]];
    ...
    //    double dblFuel = (109186/(fltDensity));
...
- (void) calcTrimSet
{
    double c = 7-(6 * (([self.txtGWCG.text doubleValue]-22)/16));
    if (c < 0) {
    ...
    if ([self.txtGWCG.text doubleValue] <= 22 && [self.txtGWCG.text doubleValue] >= 18) {
        ...
        if ([self.txtGWCG.text doubleValue] <= 40 && [self.txtGWCG.text doubleValue] >= 38) {
            self.txtTrim.text = [[[NSString alloc]initWithFormat:@"%2.2f", 1.0] stringByAppendingString:@"° UP"];
        }
    }
    if ([self.txtGWCG.text doubleValue] > 40 || [self.txtGWCG.text doubleValue] < 18) {
        self.txtTrim.text = @"XXXX";
    }
    self.lblTrim.text = @"";
    if ([self.self.txtGWCG.text doubleValue] >= 28 && [self.txtGWCG.text doubleValue] <= 40) {
...
-(IBAction)formatZFW:(id)sender
{
    if ([self.txtZFW.text length] >= 6) {
        if ([self.txtZFW.text intValue] > 181000) {
            UIAlertView *message = [[UIAlertView alloc] initWithTitle:@"Caution"
...
-(IBAction)updatesldFuel:(id)sender
{
    ...
    if ([self.navigationItem.title isEqualToString:@"Standard Fuel Distribution A340-300"]) {
...
-(IBAction)nonStdFuel:(id)sender
{
    ...
    [self.navigationItem setTitle:@"Non Standard Fuel Distribution A340-300"];
...
{
    [self.navigationItem setTitle:@"Standard Fuel Distribution A340-300"];
    ...
    if (intTotalFuel <= 9000) { // INNER TANKS 9000
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%ld ", intTotalFuel/2];
        self.progFuelInnerLeft.progress = (float)intTotalFuel/2/(float)((42774*intDensity)/1000);
        self.txtInnerFuelRight.text = [[NSString alloc] initWithFormat:@"%ld ", intTotalFuel/2];
        self.progFuelInnerRight.progress = (float)intTotalFuel/2/(float)((42774*intDensity)/1000);
        NSLog(@"%f", (float)intTotalFuel/2/(float)((42774*intDensity)/1000));
    ...
    } else if ( intTotalFuel > 9000 && intTotalFuel <= ((7300*intDensity)/1000)+9000) //OUTERS ONLY 14730
    {
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%d ", 4500];
        self.progFuelInnerLeft.progress = (float)4500/(float)((42774*intDensity)/1000);
        self.txtInnerFuelRight.text = [[NSString alloc] initWithFormat:@"%d ", 4500];
        self.progFuelInnerRight.progress = (float)4500/(float)((42774*intDensity)/1000);
        ...
        self.txtTrimFuel.text = @"0";
        self.progFuelTrimTank.progress = 0;
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+9000 && intTotalFuel <= ((7300*intDensity)/1000)+30770) //INNERS 36500
    {
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%ld ", (intTotalFuel-(((3650*intDensity)/1000)*2))/2];
        self.progFuelInnerLeft.progress = (float)(intTotalFuel-5730)/2/(float)((42774*intDensity)/1000);
        self.txtInnerFuelRight.text = [[NSString alloc] initWithFormat:@"%ld ", (intTotalFuel-(((3650*intDensity)/1000)*2))/2];
        //        self.txtInnerFuelRight.text = [[NSString alloc] initWithFormat:@"%ld ", (intTotalFuel-5730)/2];
        self.progFuelInnerRight.progress = (float)(intTotalFuel-5730)/2/(float)((42774*intDensity)/1000);
        self.txtOuterFuelLeft.text = [[NSString alloc] initWithFormat:@"%li ",((3650*intDensity)/1000)];
        ...
        self.txtTrimFuel.text = @"0";
        self.progFuelTrimTank.progress = 0;
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+30770 && intTotalFuel <= ((7300*intDensity)/1000)+2400+30770) // TRIM ONLY 38900
    {
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%d ", 15386];
        self.progFuelInnerLeft.progress = (float)15386/(float)((42774*intDensity)/1000);
        self.txtInnerFuelRight.text = [[NSString alloc] initWithFormat:@"%d ", 15386];
        self.progFuelInnerRight.progress = (float)15386/(float)((42774*intDensity)/1000);
        self.txtOuterFuelLeft.text = [[NSString alloc] initWithFormat:@"%li ", ((3650*intDensity)/1000)];
        self.txtOuterFuelRight.text = [[NSString alloc] initWithFormat:@"%li ", ((3650*intDensity)/1000)];
        self.txtTrimFuel.text = [[NSString alloc] initWithFormat:@"%ld ", (intTotalFuel-36500)];
        self.progFuelTrimTank.progress = (float)(intTotalFuel-36500)/(float)((6230*intDensity)/1000);
        self.txtCenterFuel.text = @"0";
        self.progFuelCenterTank.progress = 0;
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+2400+30770 && intTotalFuel <= ((7300*intDensity)/1000)+((85548*intDensity)/1000)+2400) //INNERS ONLY 75286
    {
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%ld ", (intTotalFuel-(((7300*intDensity)/1000)+2400))/2];
        self.progFuelInnerLeft.progress = (float)((intTotalFuel-(((7300*intDensity)/1000)+2400))/2)/(float)((42774*intDensity)/1000);
        NSLog(@"%f", (float)((intTotalFuel-(((7300*intDensity)/1000)+2400))/2)/(float)((42774*intDensity)/1000));
        self.txtInnerFuelRight.text = [[NSString alloc] initWithFormat:@"%ld ", (intTotalFuel-(((7300*intDensity)/1000)+2400))/2];
        self.progFuelInnerRight.progress = (float)((intTotalFuel-8130)/2)/(float)((42774*intDensity)/1000);
        self.txtOuterFuelLeft.text = [[NSString alloc] initWithFormat:@"%li ", ((3650*intDensity)/1000)];
        self.progFuelOuterLeft.progress = 1;
        self.txtOuterFuelRight.text = [[NSString alloc] initWithFormat:@"%li ", ((3650*intDensity)/1000)];
        self.progFuelOuterRight.progress = 1;
        self.txtTrimFuel.text = [[NSString alloc] initWithFormat:@"%d ", 2400];
        self.progFuelTrimTank.progress = (float)2400/(float)((6230*intDensity)/1000);
        self.txtCenterFuel.text = @"0";
        self.progFuelCenterTank.progress = 0;
    } else if ( intTotalFuel > ((7300*intDensity)/1000)+((85548*intDensity)/1000)+2400 && intTotalFuel <= ((141500*intDensity)/1000)) // CENTER & TRIM TO FULL 111076
    {
        self.txtInnerFuelLeft.text = [[NSString alloc] initWithFormat:@"%li ", ((42774*intDensity)/1000)];
        self.progFuelInnerLeft.progress = 1;
        self.txtInnerFuelRight.text = [[NSString alloc] initWithFormat:@"%li ", ((42774*intDensity)/1000)];
        ...
        NSInteger intFueled = (((42774*intDensity)/1000)*2)+(((3650*intDensity)/1000)*2)+2400;
        self.txtTrimFuel.text = [[NSString alloc] initWithFormat:@"%1.0f ", (((intTotalFuel-intFueled)*6.956)/100)+2400];
        self.progFuelTrimTank.progress = (float)((((intTotalFuel-intFueled)*6.956)/100)+2400)/(float)((6230*intDensity)/1000);
        
        self.txtCenterFuel.text = [[NSString alloc] initWithFormat:@"%1.0f ",(intTotalFuel - intFueled) - ((intTotalFuel-intFueled)*6.956)/100];
        NSInteger intfuelonboard = (((42774*intDensity)/1000)*2) + (((3650*intDensity)/1000)*2) + 2400;
        self.progFuelCenterTank.progress = (float)((intTotalFuel - intfuelonboard) - ((intTotalFuel-intfuelonboard)*6.956)/100)/(float)((42420*intDensity)/1000);
    }
...
-(void)calculateIndex
{
    ...
    NSLog(@"%@", ([[NSString alloc] initWithFormat:@"%1.0ld",(long)intIndex]));
    NSLog(@"%1.0ld",(long)intIndex);
...
-(void)indexInnerTankLeft
{
    ...
    //    }
    self.progFuelInnerLeft.progress = (float)intInnerFuel/(float)((42000*intDensity)/1000);
    NSLog(@"%f", (float)intInnerFuel/(float)((42774*intDensity)/1000));
}
...
-(void)indexTrimTank
{
    ...
    if (intTrimFuel==0) {
        ...
        if (intDensity >= 805) {
            self.lblIndexTrimTank.text = @"+56";
        }else {
            self.lblIndexTrimTank.text = @"+57";
        }
    } else if (intTrimFuel<=5000) {
        self.lblIndexTrimTank.text = @"+58";
    } else if (intTrimFuel<=5100) {
        self.lblIndexTrimTank.text = @"+59";
    } else if ((float)intTrimFuel==((6230*intDensity)/1000)) {
        if (intDensity <= 770) {
            self.lblIndexTrimTank.text = @"+55";
        }else if (intDensity <= 785) {
            self.lblIndexTrimTank.text = @"+56";
        }else if (intDensity <= 795) {
            self.lblIndexTrimTank.text = @"+57";
        }else if (intDensity <= 810) {
            self.lblIndexTrimTank.text = @"+58";
        }else if (intDensity <= 825) {
            self.lblIndexTrimTank.text = @"+59";
        }else if (intDensity <= 830) {
            self.lblIndexTrimTank.text = @"+60";
        }
...
-(void)indexInnerTankRight
{
    ...
    //    }
    self.progFuelInnerRight.progress = (float)intInnerFuel/((42000*intDensity)/1000);
}
