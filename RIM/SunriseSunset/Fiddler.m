...
@synthesize latitude,longitude,timeZone,date,sunrise,sunset,Fajr,Isha;
...
- (void) setupSunriseSunset  {
	...
	unsigned unitFlags = NSCalendarUnitHour | NSCalendarUnitMinute |  NSCalendarUnitSecond | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitYear;
	
	// Fajr
	double utc = [self calcSunriseUTC];
    double utcFajr = [self calcFajrUTC];
    NSDate *gmtFajr = [[NSDate alloc] initWithTimeIntervalSinceReferenceDate:(utcFajr * 60.0)];
	NSDateComponents *compsFajr = [[NSCalendar currentCalendar] components:unitFlags fromDate:gmtFajr];
	NSLog(@"Fajr = %ld/%ld/%ld %ld:%ld:%ld", (long)compsFajr.month, (long)compsFajr.day, (long)compsFajr.year, (long)compsFajr.hour, (long)compsFajr.minute, (long)compsFajr.second);
    
    self.Fajr = [[NSCalendar currentCalendar] dateFromComponents:compsFajr];	
	...
	NSLog(@"sunrise = %ld/%ld/%ld %ld:%ld:%ld", (long)comps.month, (long)comps.day, (long)comps.year, (long)comps.hour, (long)comps.minute, (long)comps.second);
    ...
    double utcIsha = [self calcIshaUTC];
    NSDate *gmtIsha = [[NSDate alloc] initWithTimeIntervalSinceReferenceDate:(utcIsha * 60.0)];
	NSDateComponents *compsIsha = [[NSCalendar currentCalendar] components:unitFlags fromDate:gmtIsha];
	
    self.Isha = [[NSCalendar currentCalendar] dateFromComponents:compsIsha];
	...
	NSLog(@"sunset = %ld/%ld/%ld %ld:%ld:%ld", (long)comps.month, (long)comps.day, (long)comps.year, (long)comps.hour, (long)comps.minute, (long)comps.second);
    NSLog(@"Isha = %ld/%ld/%ld %ld:%ld:%ld", (long)compsIsha.month, (long)compsIsha.day, (long)compsIsha.year, (long)compsIsha.hour, (long)compsIsha.minute, (long)compsIsha.second);
...
-(double)calcFajrUTC {
	return [self calcFajrUTCForDegree:108.000];
}
...
-(double)calcFajrUTCForDegree:(double) degAngle {
	double jd = [self calcJD];
	...
	double timeUTCFajr = 720 + timeDiff - eqTime;	// in minutes
    
    //	NSLog(@"eqTime = %f\nsolarDec = %f\ntimeUTC = %f", eqTime, solarDec, timeUTC);
	
	// *** Second pass includes fractional jday in gamma calc
	double newt = [self calcTimeJulianCent:([self calcJDFromJulianCent:t] + timeUTCFajr/1440.0)]; 
	eqTime = [self calcEquationOfTime:newt];
	solarDec = [self calcSunDeclination:newt];
	hourAngle = [self calcHourAngleSunrise:latitude withSolarDec:solarDec degAngle:degAngle];
	...
	delta = longitude - [self radToDeg:hourAngle];
	timeDiff = 4 * delta;
	timeUTCFajr = 720 + timeDiff - eqTime; // in minutes
...
-(double)calcIshaUTC {
	return [self calcIshaUTCForDegree:107.000];
}
...
-(double) calcIshaUTCForDegree:(double) degAngle {
	double jd = [self calcJD];
	double t = [self calcTimeJulianCent:jd];
	...
	double timeUTCIsha = 720 + timeDiff - eqTime;	// in minutes
	
    //	NSLog(@"eqTime = %f\nsolarDec = %f\ntimeUTC = %f", eqTime, solarDec, timeUTC);
	
	// *** Second pass includes fractional jday in gamma calc
	double newt = [self calcTimeJulianCent:([self calcJDFromJulianCent:t] + timeUTCIsha/1440.0)]; 
	eqTime = [self calcEquationOfTime:newt];
	solarDec = [self calcSunDeclination:newt];
	hourAngle = [self calcHourAngleSunset:latitude withSolarDec:solarDec degAngle:degAngle];
	...
	delta = longitude - [self radToDeg:hourAngle];
	timeDiff = 4 * delta;
	timeUTCIsha = 720 + timeDiff - eqTime; // in minutes
